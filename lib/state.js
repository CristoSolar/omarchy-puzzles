...
var DEFAULT_STATE = { lastSolved: '', streak: 0, lastElapsedMs: 0, best: {}, inProgress: null };

function freshState() {
  return { lastSolved: '', streak: 0, lastElapsedMs: 0, best: {}, inProgress: null };
}
...
function parseState(text) {
  ...
  if (typeof raw.lastElapsedMs === 'number' && isFinite(raw.lastElapsedMs) && raw.lastElapsedMs >= 0) {
    state.lastElapsedMs = raw.lastElapsedMs;
  }
...
// La barra y el panel preguntan esto en vez de comparar fechas cada uno por su
// cuenta: el panel lo usa para no ofrecer de nuevo un puzzle ya resuelto tras
// reiniciar el shell o al abrirlo en otro monitor.
function solvedOn(state, dateKey) {
  return state.lastSolved === dateKey && dateKey !== '';
}
